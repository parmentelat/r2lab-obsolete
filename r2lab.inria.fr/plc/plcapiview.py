...
class PlcApiView(TestbedApiView):
    ...
    def init_plcapi_proxy(self):
        ...
        for credentials in plcapi_settings['credentials']:
            try:
                ...
                pass
        if not found:
            print("Cannot find credentials to use for plcapi")
            for credentials in plcapi_settings['credentials']:
                print("have tried in {}".format(credentials))

        self.plcapi_proxy \
            = PlcApiProxy(plcapi_settings['url'],
                          email=email,
                          password=password,
                          debug=debug,
            )
    ...
    def ui_ts_to_plc_ts(self, ui_timestamp):
        return ui_timestamp.replace('T', ' ').replace('Z', '')
