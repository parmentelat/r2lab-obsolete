...
def wifi_ips():
    "returns the ips confs (net_mask, ip_sender, ip_receiver)"
    return '/24', '172.16.1.1', '172.16.1.2'
...
def one_run(gwhost, gwuser, key, sendername, receivername, file, port, storage):
    ...
    dataname = os.path.join(storage, "wifiout-{}-{}-{}"
    ...
    # get wifi confs
    wifi_interface, wifi_channel, wifi_name, wifi_key = wifi_conf()

    # get wifi ip's
    wifi_net_mask, wifi_ip_sender, wifi_ip_receiver = wifi_ips()
    ...
    ec = ExperimentController(exp_id="B2-send-file-shift")
    ...
    # configuring the ad-hoc for node sender
    ...
    cmd += "sudo ip addr add {}{} dev {}; ".format(wifi_ip_sender, wifi_net_mask, wifi_interface)
    app_adhoc_sender = ec.register_resource("linux::Application",
                                            command = cmd,
                                            autoDeploy = True,
                                            connectedTo = sender)

    # configuring the ad-hoc for node receiver
    cmd  = "ip addr flush dev {}; ".format(wifi_interface)
    cmd += "sudo ip link set {} down; ".format(wifi_interface)
    cmd += "sudo iwconfig {} mode ad-hoc; ".format(wifi_interface)
    cmd += "sudo iwconfig {} channel {}; ".format(wifi_interface, wifi_channel)
    ...
    cmd += "sudo iwconfig {} key {}; ".format(wifi_interface, wifi_key)
    cmd += "sudo ip link set {} up; ".format(wifi_interface)
    cmd += "sudo ip addr add {}{} dev {}; ".format(wifi_ip_receiver, wifi_net_mask, wifi_interface)
    app_adhoc_receiver = ec.register_resource("linux::Application",
                                            command = cmd,
                                            autoDeploy = True,
    ...
    ec.deploy([run_local, run_gateway, run_sender, app_adhoc_sender, run_receiver, app_adhoc_receiver, run_ls_receiver])
