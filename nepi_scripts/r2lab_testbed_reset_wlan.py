...
for hostname in hosts:
    ...
    ec.register_condition(node, ResourceAction.DEPLOY, reboot_app, 
            ResourceState.STOPPED, time="20s") 
    ...
    ec.register_condition(ping_app, ResourceAction.START, 
            reboot_app, ResourceState.STOPPED, time="40s")
    modprobe_app = ec.register_resource("linux::Application")
    ec.set(modprobe_app, "command", "modprobe ath9k; lsmod | grep ath")
    ec.register_connection(modprobe_app, node)
    apps.append(modprobe_app)
    ec.register_condition(modprobe_app, ResourceAction.START, 
            ping_app, ResourceState.STOPPED, time="20s") 
    wlan_start_app = ec.register_resource("linux::Application")
    wlan_start_cmd = "iwconfig wlan0 mode ad-hoc channel 03 essid fitessai; ip link set wlan0 up; ip addr add 172.16.1.%s/24 dev wlan0" % host[-2:]
    ...
    ec.register_condition(wlan_start_app, ResourceAction.START, 
            modprobe_app, ResourceState.STOPPED, time="20s") 
    ...
    ec.register_condition(wlan_check_app, ResourceAction.START, 
            wlan_start_app, ResourceState.STOPPED, time="20s") 
